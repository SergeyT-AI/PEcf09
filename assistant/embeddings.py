...
def _ensure_modern_sqlite() -> None:
    """ChromaDB needs sqlite3 >= 3.35; Ubuntu 20.04 ships 3.31."""
    try:
        import sqlite3
...
class EmbeddingStore:
    ...
    def __init__(
        ...
        from openai_client import make_openai_client, openai_base_url, use_proxyapi
        ...
        # Эмбеддинги через ProxyAPI (OpenAI-совместимый endpoint) или напрямую
        self.openai_client = make_openai_client(api_key)
        self.embedding_model = embedding_model
        backend = "ProxyAPI" if use_proxyapi() else "OpenAI"
        print(f"Модель эмбеддингов: {embedding_model} ({backend}: {openai_base_url()})")
